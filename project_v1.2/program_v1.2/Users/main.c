...
uint8_t g_BeepFlag = 0;
uint8_t g_UpdateFlag = 1;
double pre_temperature = 0;
uint32_t OUT_PWM = 40;
// 在全局变量区域添加以下变量
uint32_t rainbow_index = 0;  // 流水灯索引
    ...
    while (1)
    {
        ...
        if (enc_delta != 0) {
            int16_t delta = 0;
					...
					// 临界区保护
						HAL_NVIC_DisableIRQ(EXTI9_5_IRQn);   // 关编码器中断（A相）
						HAL_NVIC_DisableIRQ(EXTI15_10_IRQn);   // 关编码器中断（B相）
						delta = enc_delta;
						enc_delta = 0;
            ...
            // 更新参数（带限幅）
            if (delta > 0 && temp_set < 300) temp_set++;
            if (delta < 0 && temp_set > 0)   temp_set--;
        ...
        // === 2. 处理按键消抖（软件消抖，精准可控）===
        if (sw_press) {
            if (HAL_GetTick() - last_sw_time > 20) { // 20ms去抖
                if (HAL_GPIO_ReadPin(EN_GPIO_PORT, EN_SW_GPIO_PIN) == 0) {
                    // 确认为有效按键
            ...
            last_sw_time = HAL_GetTick();
        }

        // === 3. 执行蜂鸣器请求（非阻塞）===
        if (beep_req) {
            Buzzer_Tone(10); // 调用我们之前设计的非阻塞函数
            beep_req = 0;
        }
...
{
    uint8_t hue = position % 27; // 修改为27，与实际灯数匹配
    
    // 将27个位置均匀分布在HSB色彩环上
    if(position < 5) {
        // 红到黄过渡 (约0-60度)
        color->R = 0xFF;
        color->G = position * 51;  // 0-255
        color->B = 0x00;
    }
    else if(position < 9) {
        // 黄到绿过渡 (约60-120度)
        color->R = 0xFF - ((position-5) * 64); // 255-0
        color->G = 0xFF;
        color->B = 0x00;
    }
    else if(position < 14) {
        ...
        color->R = 0x00;
        color->G = 0xFF;
        color->B = (position-9) * 51; // 0-255
    }
    else if(position < 18) {
        // 青到蓝过渡 (约180-240度)
        color->R = 0x00;
        color->G = 0xFF - ((position-14) * 64); // 255-0
        color->B = 0xFF;
    }
    else if(position < 23) {
        // 蓝到紫过渡 (约240-300度)
        color->R = (position-18) * 51; // 0-255
        color->G = 0x00;
        color->B = 0xFF;
    }
    else {
        ...
        color->R = 0xFF;
        color->G = 0x00;
        color->B = 0xFF - ((position-23) * 51); // 255-0
    }
}
    ...
    switch(rgb_current_state)
    {
        case RGB_STATE_RAINBOW:
            ...
            if(rgb_state_changed || 1) // 总是更新以保持动画效果
            {
                for(int i = 0; i < 27; i++)
                {
                    uint8_t color_position = (i + rainbow_index) % 27;
                    Color_TypeDef color;
        ...
        case RGB_STATE_BLUE:
            ...
            {
                WS2812B_FillColor(0, 27, &temp2);  // 使用冰蓝色
                WS2812B_RefreshPixel();
                rgb_state_changed = 0;
            }
            break;
        ...
        case RGB_STATE_YELLOW_GREEN:
            ...
            if(rgb_state_changed)
            {
                WS2812B_FillColor(0, 27, &temp3);  // 使用浅黄绿色
                WS2812B_RefreshPixel();
                rgb_state_changed = 0;
            }
            break;
        ...
        case RGB_STATE_YELLOW:
            ...
            if(rgb_state_changed)
            {
                WS2812B_FillColor(0, 27, &temp4);  // 使用黄色
                WS2812B_RefreshPixel();
                rgb_state_changed = 0;
            }
            break;
        ...
        case RGB_STATE_ORANGE:
            ...
            if(rgb_state_changed)
            {
                WS2812B_FillColor(0, 27, &temp5);  // 使用橙色
                WS2812B_RefreshPixel();
                rgb_state_changed = 0;
            }
            break;
        ...
        case RGB_STATE_RED:
            ...
            if(rgb_state_changed)
            {
                WS2812B_FillColor(0, 27, &temp6);  // 使用红色
                WS2812B_RefreshPixel();
                rgb_state_changed = 0;
            }
            break;
        ...
        case RGB_STATE_PURPLE:
            ...
            if(rgb_state_changed)
            {
                ...
                WS2812B_FillColor(0, 27, &purple_color);
                WS2812B_RefreshPixel();
                rgb_state_changed = 0;
            }
            break;
